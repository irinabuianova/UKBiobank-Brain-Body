...
# Load data
base_path = '/mnt/UK_BB/brainbody/body'
...
save_path = '/mnt/UK_BB/brainbody/lifestyle-envir'
folds_save_path = os.path.join(save_path, f'folds/fold_{folds[fold]}')
models_path = os.path.join(folds_save_path, 'models')
g_pred_path = os.path.join(folds_save_path, 'g_pred')

for path in [folds_path, suppl_path, scaling_path, save_path, folds_save_path, models_path, g_pred_path]:
    os.makedirs(path, exist_ok=True)
    print(f"Directory checked/created: {path}", flush=True)

# Upload targets and eid
targets_train = pd.read_csv(os.path.join(suppl_path, f'{modalities[modality]}_train_targets_fold_{folds[fold]}.csv'), usecols = ['eid', 'g'])
targets_test = pd.read_csv(os.path.join(suppl_path, f'{modalities[modality]}_test_targets_fold_{folds[fold]}.csv'), usecols = ['eid', 'g'])
...
if num_features > 30000:  # Use RandomizedSearchCV for large feature sets
...
# Predict and save
g_pred_test = save_predictions(features_test_scaled, g_test, eid_test, 'test', modalities[modality], fold)
g_pred_train = save_predictions(features_train_scaled, g_train, eid_train, 'train', modalities[modality], fold)
