...
CONNECTOMES_DIR = os.path.join(BASE_PATH, "brain/data/dwMRI/connectomes")


modality_folders = {
'aparc_a2009s_Tian_S1': '31020_aparc_a2009s_Tian_S1',
'aparc_Tian_S1': '31021_aparc_Tian_S1',
'Glasser_Tian_S1': '31022_Glasser_Tian_S1',
'Glasser_Tian_S4': '31023_Glasser_Tian_S4',
'Schaefer7n1000p_Tian_S4': '31024_Schaefer7n1000p_Tian_S4',
'Schaefer7n200p_Tian_S1': '31025_Schaefer7n200p_Tian_S1'
}
...
def find_modality_file(modality_name):
    """Find the correct file path for a given modality"""
    matched_folder = None
    for folder_key in modality_folders:
        if folder_key in modality_name:
            matched_folder = modality_folders[folder_key]
            break
    
    if not matched_folder:
        raise ValueError(f"No matching folder found for modality: {modality_name}")
    
    file_path = os.path.join(CONNECTOMES_DIR, matched_folder, f"{modality_name}.csv")
...
def deconfound_dwi(modality, fold):
    ...
    try:
        ...
        features = pd.read_csv(find_modality_file(modality))
        confounds = pd.read_csv(os.path.join(BASE_PATH, 'brain/data/dwMRI/dwi_conf.csv'))
...
if __name__ == "__main__":
    ...
    folds = range(5)
    jobs = [(mod, fold) for mod in dwi_modalities for fold in folds]
